...
Shopify.theme.mobileMenu = {
  ...
  open: function () {
    ...
    let mobileHeaderHeight = parseInt($('.mobile-header').outerHeight()) + parseInt($('.top-bar').outerHeight());
console.log(mobileHeaderHeight)
  ...
  enableSticky: function () {
    ...
    $stickyEl.sticky({
      ...
      var annoucementHeight = $('.announcement-sticky-wrapper').outerHeight();
    ...
    $.ajax({
      ...
      error: function (XMLHttpRequest, textStatus) {
        ...
        if($("[data-line-item='"+lineID+"'] .quantity-warning").length > 0){
          ...
          $("[data-line-item='"+lineID+"']  input.quantity-input").val(parseInt(quantity-1));
          setTimeout(function(){
...
Shopify.theme.handleNewTabLinks = function ($links) {
  if (!$links.length) return;
  $links.each(function (index, link) {
    let href = $(link).attr('href')
    if(Shopify.theme.isExternalLink(href)){
      link.setAttribute('target','_blank')
    }
  })
};

Shopify.theme.isExternalLink = function(link){
